...
class SimulationView(arcade.View):      
    ...
    def on_draw(self):
        ...
        if not self.graph == None:
            arcade.draw_texture_rectangle(1000,500,self.graph.get_size_inches()[0]*self.graph.dpi,self.graph.get_size_inches()[1]*self.graph.dpi,self.graph_texture)
    ...
    def on_update(self, delta_time):
        ...
        if self.clock.new_day:
            plt.bar(self.stats.populations.keys(),self.stats.populations.values())
            self.graph = plt.gcf()
            self.graph.set_size_inches(2,2)
            self.graph.savefig(fname=os.path.join(".","output","graph_a"))
            self.graph_texture = arcade.load_texture(os.path.join(".","output","graph_a.png"))
